...
- **T1** `config.py` — `ScannerEvolveConfig` + `SCANNER_ADJUSTABLE` (10 paths: high_breakout.window, ma_cross.fast/slow, gap.threshold, rsi_divergence.div_window, severity_mult.*, top_n). Bounds derived to contain the real detector defaults (plan's illustrative bounds were inconsistent). validate enforces cross-field `fast<slow` + fixed-kernel `quant_weight=0`/`event_weight=1`. Commit `57f67bd`.
- **T2** `_detectors_from_config` + parametrized `MaCrossDetector` (added `fast/slow/lookback_days` ctor; was module-constants-only). Derived `lookback_days=max(400,param*2+100)` so max window/slow still gets enough bars. Existing ma_cross tests stay green. Commit `4e87a12`.
- **T3** `scanner_fitness` — A/B-vs-random `diff` over a regime via a `_UniverseAsOfClient` (routes per-ticker `CachedAsOfClient`, no-lookahead). Highest-risk task. No-lookahead test hardened to a clamp-vs-error discriminator (mutation-verified). Commits `b614def`, `f038eed`.
- **T4** prefetch-once parse cache (shared `cache` dict; pure perf optimization, results identical with/without). Commit `58184a5`.
- **T5** `samples.py` — immutable train(bear+bull)/val(choppy)/test(held-out 2025-09-01..2026-06-01) sample isolation. Commit `371c1fe`.
- **T6** `run.py` — `evolve_scanner` + CLI. **Engine seams** added to `v2/self_evolve/loop.py` (`apply_delta_fn`/`rebuild_config_fn`/`keep_fn`) + `proposer.py` (`adjustable`/`apply_delta`), all defaulting to factor behavior → factor suite green (verified by stash diff). Scanner keep rule = diff-improves + n_fired-floor + t_stat-not-worse. "test" never passed in the loop (recorder-asserted). Commit `312b33d`.
- **T7** `report.py` (md+html: optimization path read from version records, retained config, TEST verdict, honest caveats) + offline end-to-end `test_smoke.py` (md+html exist, TEST renders, "test" never in loop). Smoke bounded to ~44s. Commits `f96298b`, `6f77b67`.

### Tests
- `v2/scanner/evolve/` — 54 passed. `v2/self_evolve/` — 161 passed, 1 failed (pre-existing `test_graduate::test_load_best_config_real_strategy_skill_dir_loads`, a 5-vs-11-factor mismatch, identical with/without this work — confirmed via stash). `v2/scanner/` — 284 passed, 2 skipped (incl. the now-fast smoke).
- Final whole-feature review: READY TO MERGE; all 4 load-bearing invariants hold end-to-end; factor engine intact.

### Known minor limitation (not a bug)
`run.main`'s prefetch span pads a fixed 400 calendar days before the earliest sample start; if the proposer pushes `high_breakout.window`/`ma_cross.slow` to their max (300), the very earliest as-of dates lack enough history → those detectors return `None` (graceful, no-lookahead-preserving coverage shrink at window edges). Bump `_LOOKBACK_DAYS`→~700 if full-coverage at extreme configs is wanted.

### Not yet done
On `main` locally, NOT pushed to origin. No real evolve run executed yet (needs API keys + the live scanner forward-test is the real judge).
...
- C4 (overnight): RsiDivergenceDetector — price-vs-RSI(14) classic divergence. Wilder RSI implemented inline (BaseSignal._compute_rsi uses simple rolling mean, not Wilder). Two-half swing detection over 40-bar window. Bearish (higher price high + lower RSI high) and bullish (lower price low + higher RSI low). Severity = RSI gap / 10, capped at 8 (coefficient, no z-divisor). None on <55 bars or NaN RSI in window; triggered=False on no divergence; degenerate flat → no raise. Registered in ALL_DETECTORS + DETECTOR_METADATA + __all__. README updated. 7 new tests green. v2/scanner suite 144/2 (was 137/2). Findings.md C4 entry added. Commit feat(scanner): RSI divergence detector.
...
- C6: Wave C verification — v2/scanner/ 156 passed 2 skipped; registry now 13 detectors, all 4 new (high_breakout, gap, ma_cross, rsi_divergence) present in ALL_DETECTORS by .name + DETECTOR_METADATA; eval package imports clean (forward_return/evaluate_detector/run_ab). All 4 detectors passed scanner-invariant-reviewer gate (APPROVED). Wave C complete: 4 detectors + A/B eval harness, 0 regressions.
...
## Multi-tenant accounts (branch: feature/multi-tenant-accounts)

- **Wave 0 — Postgres foundation:** `DATABASE_URL` from env w/ sqlite fallback (`connection.py`), alembic `env.py` honors it; deps added (passlib, python-jose, authlib, psycopg2-binary, email-validator). PG migration smoke **deferred** (Docker Desktop engine was down) — steps in `docs/superpowers/notes/postgres-bringup.md`; build continues on SQLite per plan.
- **Wave 1 — Auth core:** `User`+`OAuthAccount` models + migration `9bc3783803fc` (alembic-reviewer APPROVED); bcrypt hashing; JWT access/refresh; `UserRepository` (+`find_or_create_oauth`); auth schemas; `/auth` register/login/refresh/logout/me + `get_current_user`. Security review APPROVED (no criticals); applied password `min_length=8`, refresh-cookie rotation on `/refresh`, token-type-confusion test. **Deferred to Wave 8:** prod `JWT_SECRET` startup assertion. **Carry to Wave 2 (security):** OAuth must only auto-link to an existing password account when the provider has *verified* the email (account-takeover guard — review finding M-7). `tests/auth` 23 passed; full suite 960 passed / 5 skipped.

- **DB decision revised (2026-05-29):** stay on **SQLite + WAL + busy_timeout=30s** instead of migrating to Postgres — sufficient for a few friends, far simpler to deploy/back up. `connection.py` sets the pragmas via a sqlite-only connect event. Postgres remains a zero-code-change switch via `DATABASE_URL` (Wave-0 plumbing). Wave 0 Task 0.3 PG smoke → optional/future; the Docker dependency is no longer on the critical path.
- **Wave 2 — OAuth:** `app/backend/auth/oauth.py` (sync Google/GitHub: provider registry, `build_authorize_url`, httpx `exchange_code` returning `email_verified`). `/auth/oauth/{provider}` (state cookie → 302) + `/callback` (state validation, **rejects unverified provider email** per review M-7, `find_or_create_oauth`, refresh cookie + fragment access token → frontend). 7 new tests (config + routes incl. unverified-email + bad-state). Full suite 969 passed / 5 skipped. Commits ee538b6, b36a1ac. → Auth subsystem (Waves 0–2) complete.
- **Wave 3 — tenancy columns:** Task 3.1 added nullable `user_id` to 11 owned tables + seeded owner (id=1, `owner@local`) + backfilled all rows; migration `359c3b216a47` (alembic-reviewer APPROVED). Confirmed SQLite 3.45 evaluates `server_default text("true")` → 1 (new users active). Task 3.2 (NOT NULL) implemented then **reverted (`cf26994`) and re-sequenced to Task 4.12** — enforcing NOT NULL before any writer sets `user_id` red-lined 154 existing tests; the column stays nullable through Wave 4 so the suite stays green task-by-task. Suite 970 passed / 5 skipped; head `359c3b216a47`.
- **Wave 4 schema prep:** Task 4.1 (watchlists) surfaced two schema gaps when scoping the next resources: (1) `screener_presets` had **no `user_id`** — it was omitted from the Wave-3.1 list (should've been 12 owned tables, not 11) → migration `17da9147c7cb` adds it + backfills owner; (2) `analyze_flows.name` was globally `unique=True` → migration `591dc9226a2d` makes it per-user composite unique (like watchlists). Both self-verified (up/down/up clean, schema matches models, create_all parity, suite 976 green). Head `591dc9226a2d`.
- **Wave 4 — scanner + pipeline (last two resources):** **scanner_configs** (`1a36110`): `ScannerConfigRepository` create/get/update/delete scoped by `user_id` (+`list_for_user`); unscoped `get_by_id_unscoped`/`list_all` kept for `ScannerService` + `SchedulerService` startup (registers ALL users' enabled crons). `ScanRunRepository.get_by_id_for_user` joins ScanRun→ScannerConfig and filters by config owner (ScanRun/WatchlistEntry have no `user_id` — scoped via parent FK). Routes inject `get_current_user`; cross-tenant → 404. Cron/manual run path now threads `config.user_id` through the payload so auto-SOP follow-up reports are attributed to the owner. `tests/auth/test_isolation_scanner.py` (9). — **pipeline** (`3441d2a`): `PipelineRunRepository` create stamps `user_id`; get/list scope by it (lifecycle mark_* stay unscoped, keyed by unique run_id). `pipeline_schedule` **singleton→per-user**: `get_or_create_for_user`/`update_for_user` back the routes (lazy create on first GET), legacy unscoped `get()` kept for cron-fallback + notification dispatcher. Cron `_run_pipeline_job` resolves the seed superuser, uses that owner's schedule + stamps the run with owner_id (falls back to `get()`+unowned when no superuser); Wave-6 note added. `main.py` dropped the global id=1 seed. **Schema note (flagged, no migration):** `PipelineSchedule.id` carried a client-side `default=1` (singleton artifact) that forced every per-user insert to id=1 → removed it; SQLite `INTEGER PRIMARY KEY` autoincrements natively so no DDL/Alembic change was needed. `tests/auth/test_isolation_pipeline.py` (6). Full suite **1026 passed / 5 skipped / 0 failed**.
- **Wave 7 — Frontend auth (commits 708e47b, fa99f6d):** login/register page + Google/GitHub buttons, `AuthProvider` (access token in localStorage, on-load `/auth/me` bootstrap, OAuth `#access_token` fragment consume), global `window.fetch` interceptor that attaches the Bearer token to API-base requests (skips /auth/* + public snapshot routes), 401→logout, protected shell in App.tsx (loading/login/authed), user menu in top-bar, i18n auth.* (en+zh). tsc clean. Dev note: cross-origin (5173↔8001) so no silent cookie-refresh — 30-min access token, re-login on expiry. **The app is now fully browser-testable with per-user auth + data isolation** (verified live: alice/bob isolation passed). Remaining: Wave 4.12 NOT NULL, Wave 5 per-user keys, Wave 6 crons, Wave 8 hardening.
- **Wave 5 — Per-user API keys (commit 9294a2e):** API-key storage is now private per user. `ApiKeyRepository`: every method takes a required `*, user_id: int` and filters `ApiKey.user_id == user_id` (upsert's find-existing scoped to the user, matching the new composite unique). `ApiKeyService(db, user_id)`: bound to one acting user; `get_api_keys_dict()`/`get_api_key(provider)` resolve only that user's keys. `routes/api_keys.py`: all 8 routes inject `get_current_user` + pass `current_user.id`; cross-tenant get/delete → 404. **Key-resolution policy (the point):** the acting user's stored key always wins; a user with NO key for a required provider gets a friendly `ApiKeyError` via `require_api_key()` ("Add your <provider> API key in Settings") — **no silent fallback to the host's `.env` key**. EXCEPTION: the seed superuser/owner (`is_superuser=1`) MAY fall back to `.env` so the host's own usage keeps working (documented in `api_key_service.py`). No background/cron caller constructs `ApiKeyService`/`ApiKeyRepository` today (cron LLM/data keys come from `.env` directly), so nothing to rewire there this wave. Migration `75ec58cc6c14` (drop global UNIQUE(provider) → composite unique `(user_id, provider)`) + models.py folded into the commit. `tests/auth/test_api_keys_per_user.py` (10: route isolation + service resolution + policy). **Frontend: no change** — Settings api-keys calls go through the Wave-7 fetch interceptor (Bearer token attached) so they're automatically per-user. Full suite **1036 passed / 5 skipped / 0 failed**. Remaining: Wave 4.12 NOT NULL, Wave 6 crons, Wave 8 hardening.
- **Wave 5 KNOWN GAP (flagged for the user):** per-user key *storage + retrieval* is scoped (commit 9294a2e), but the analyze/scan **pipeline does NOT consume DB-backed keys** — `v2.pipeline` / agents / `get_model` read provider keys from the host process `.env` (os.environ), and `src/tools/api.py` ignores passed `api_key` params. So friends can SAVE their own keys, but actual LLM/data calls still run on the HOST's `.env` keys (host still pays). Fully wiring "每人自带 key" requires threading each request's user keys down into the model/data clients (a per-request env override is unsafe — the analyze background threads would race on global os.environ). This is a deeper separate effort; surfaced to the user 2026-05-29 for a priority decision.
- **Wave 4 COMPLETE (Task 4.12, commit cbc6006):** `user_id NOT NULL` enforced on all 12 owned tables via migration `58ae3db295eb` (batch_alter per table, defensive owner-backfill first, indexes preserved, up/down/up clean). 73 tests that constructed owned models directly were fixed to pass `user_id`. Suite 1036 passed / 5 skipped / 0 failed. **Tenancy data-isolation is now airtight** — all 10 resources scoped + every owned query user-filtered + NOT NULL enforced + per-resource cross-tenant isolation tests. (Migration self-verified: chain from 75ec58cc6c14, symmetric, no NULLs pre-existing.) Remaining MT work: Wave 6 (per-user opt-in crons — mostly satisfied by Wave-4 cron adaptation; per-user-KEY usage blocked on the deferred pipeline key-wiring), Wave 8 (env CORS + JWT-secret assertion + isolation sweep).

- **Multi-tenant backend COMPLETE (Wave 6 = commit `fefbbce`; Wave 8 = this commit):** Wave 6 (`fefbbce`) verified + locked the per-user opt-in cron properties: opt-in defaults confirmed at the model level (`pipeline_schedule.enabled` default False, `screener_presets.schedule_enabled` default False; scanner configs are user-created so enabling is the opt-in) — the only always-on global job is the snapshot build, which takes no per-user keys. Fixed the one real cross-tenant leak: the preset cron's match-notify went through the dispatcher's *unscoped* `list_enabled_for_event`, so user A's preset match fanned out to EVERY user's `screener.match` subscriptions. Added owner-scoped `SubscriptionRepository.list_enabled_for_event_and_user(event_type, user_id=)` + a `user_id` param on `dispatch_screener_match`; `_run_preset_job_body` now passes `p.user_id` so a match reaches only that preset's owner. Documented the deferred per-user-API-key dependency (analyze/scan/research pipelines still read keys from host `.env`; cron runs use host keys until key-wiring lands) at the top of each per-user cron body (pipeline, research, preset). Tests: 2-user preset-cron test (each owner's match dispatches only to itself) + dispatcher scoped-lookup unit test. Wave 8 hardening: env-driven CORS in `main.py` (origins parsed from `FRONTEND_ORIGINS` comma-separated, default to the two localhost dev URLs when unset; `allow_credentials=True` kept) + JWT-secret startup guard (warn when `JWT_SECRET` unset; additionally `raise RuntimeError` when `ENVIRONMENT=production` — both verified in a subprocess; the signing default in `auth/security.py` untouched). New catch-all `tests/auth/test_isolation_sweep.py` parametrizes EVERY user-owned LIST endpoint — `/watchlists`, `/screener/presets`, `/analyze-flows`, `/lab/strategies`, `/scanner/configs`, `/notifications/subscriptions`, `/api-keys`, `/research/reports`, `/pipeline/runs` — asserting (a) unauthenticated → 401 and (b) user B's list never contains user A's row (create-then-check where POST is cheap; owner-scoped-empty for the LLM/pipeline-backed lists, whose per-row 404 isolation is covered by their dedicated modules). Full suite **1056 passed / 5 skipped / 0 failed** (was 1036/5; +20 = 18 sweep + 2 cron/dispatcher). No frontend files touched. **The multi-tenant accounts backend is now complete: auth + OAuth, per-user data isolation (airtight, NOT NULL), per-user key storage, per-user opt-in crons with owner-scoped notify, and env/secret hardening — all under a green suite.** Single remaining known gap (pre-existing, flagged Wave 5): the analyze/scan pipeline still consumes the HOST `.env` provider keys at call time; threading each request's user keys down into the model/data clients is a separate deeper effort.
- **Phase 10 Wave 2 — intraday chart (commit 48f122e, on the MT branch):** old deferred task cleared. New `src/research/charts/intraday_fetch.py::fetch_intraday_prices` (yfinance, best-effort → [] on any failure); objective-gated (short_term→5d/5m, earnings_review→1mo/15m, others skip — matches sop.md); wired into sop_orchestrator (chart_kline_intraday_b64) + html_render (<img> next to daily/weekly). 10 new tests (mocked). Suite 1066 passed / 5 skipped / 0 failed.

---

## Overnight batch #2 (2026-05-31, branch feature/multi-tenant-accounts)

All 4 tasks complete + committed. spec: docs/superpowers/specs/2026-05-31-overnight-batch-2-design.md.
1. **Watchlist pagination** (c5bb5f6) — 20/page client-side, screener-style footer.
2. **Scanner results pagination** (d5fc495) — 20/page in watchlist-table.tsx (the scan-run results), footer.
3. **Batch run reports** (114b04a) — multi-select on Screener (reused) + Watchlist (new checkboxes) → "批量跑报告" → shared BatchReportDialog (objective + market, pick once) → client concurrency pool (limit 2) over selected tickers calling the existing sync `/research/analyze` (~60-120s each), floating progress toast, notifyReportsChanged per success, cap 20. No backend route added.
...
- deploy P2-D2: docker-smoke (frontend build OK; in-process single-origin smoke with the REAL dist) caught 2 deploy-breakers: SPA not served at / (health welcome route shadowed it) + greedy catch-all shadowed trailing-slash collection routes (/api-keys). Fixed: welcome /→ /health (fast JSON for the Fly check), 404-handler SPA fallback (no shadowing). Verified + regression-tested.
- scanner round-2 action: RETIRE gap (broken z-scoring: 5σ fires ~49%, negative interestingness at every threshold). Unregistered (13→...→8 detectors); class kept + RETIRED_DETECTORS; old configs still load. Updated test. See findings_scanner_round2.md.

## Session — 2026-06-02 (per-user-keys bug fixes)

- fix auto-SOP "no key": `auto_sop_runner.run_auto_sop_for_scan` called `run_sop(req)` with NO api_keys → on the per-user-key deploy (host has no LLM keys) every scanner top-N auto-analyze report failed "no API key", while manual /research/analyze worked. Now fetches the config owner's keys via `ApiKeyService(db, owner_user_id).get_api_keys_dict()` once and threads `api_keys=` into run_sop. Audited all 3 server callers of run_sop (research route / auto_sop / Stage-3 report-schedule cron) — all now thread the owner's keys; only the local CLI (`src/research/__main__.py`) stays env-based by design. Auto-SOP top-N ran SEQUENTIALLY at this point (`for entry in entries:`). Commit 81a6e7b.
- make auto-SOP top-N CONCURRENT: two-phase — Phase 1 fans run_sop across a bounded ThreadPoolExecutor (`AUTO_SOP_MAX_WORKERS`, default 3; run_sop is thread-safe + DB-free, FastAPI already calls it concurrently across requests), Phase 2 renders+persists on the caller thread in rank order (SQLAlchemy Session is single-thread-only). Peak LLM concurrency ≈ workers × SOP_MAX_PARALLEL(10), kept modest to dodge provider rate-limits. Per-ticker `as_completed` completion log = the hook for the future analyze-progress side panel. Also fixed test regression from 81a6e7b (run_sop mocks were `lambda req:`, couldn't accept the new `api_keys=` kwarg → every report silently skipped). All 4 auto_sop tests green. Commit a75346d.
- email reports as .html ATTACHMENT not inline body: `email_report_html` now base64-attaches `{ticker}_report.html` with a one-line body pointer (mail clients truncate/rewrite big inline HTML + strip K-line chart <img> tags). `EmailHandler.send` got an optional `attachments` param (raw mode) → Resend `{filename, content(base64)}`. Added 2 handler-passthrough tests + new tests/test_report_delivery.py (2). 14 green. Commit 44600ea.
- scheduled-workflow Task 10 (Analyze "Schedule" control): toolbar Schedule button → new schedule-ticker-dialog.tsx (freq/time/lang) creates a ReportSchedule for the Input-node ticker via reportSchedulesService; shows in Settings → 定时报告. i18n en+zh. tsc clean. Commit e6faeec.
- scheduled-workflow Task 11 (Analyze concurrent-run sidebar): new contexts/analyze-runs-context.tsx (AnalyzeRunsProvider) mounted in Layout ABOVE the tab switcher → run-state survives tab switch/close. handleRun fires startRun NON-blocking (concurrent runs, removed the single-run `running` gate). New analyze-runs-sidebar.tsx (right edge): per-run live elapsed (spinner→✓/✗), click done→ReportViewerModal. Panel mirrors latest-finished run inline. Discovery: inactive tab panels are display:none (kept mounted), not unmounted. tsc clean. Commit fe55d53.
- scheduled-workflow Task 12 (Screener parallel workflow): INVESTIGATED, not implemented. ScreenerPreset is per-user + has schedule_enabled but NO cron_expr — Screener scheduling is a single GLOBAL 22:05-ET cron over all enabled presets. Per plan Open Item 1 (global-only → stop, don't invent a spec): documented in findings.md, flagged for the user as a separate spec.

## Session — 2026-06-03 (overnight: unified tracking, catalyst, report-lang, screener cron)

User asked (going to sleep, autonomous + decide-on-issues): (A) scanner/screener analyses also tracked in the analyze side panel; (B) catalyst section missing from Analyze; (C) report language should follow UI language; (D) Screener gets per-user scheduling. All 4 done.

- C report-lang-follows-UI: input-node `DEFAULT_INPUT_NODE_DATA.report_language` was pinned 'en' so the existing follow-UI fallback never fired. Dropped it (unset → follow UI) + new `lib/ui-language.ts` `uiReportLanguage()` (zh-* UI → zh) used by handleRun/handleSchedule. Commit cf5c472.
- B catalyst-in-default: catalyst was registered (backend + frontend SECTION_ORDER) + addable, but absent from `default-template.ts` PARALLEL_SECTIONS → fresh canvas had no catalyst node. Added it (11 parallel, grid 2×up-to-6). Commit c6ab445.
...
- T2 `AsOfDispatcher` (asof_dispatcher.py) — multi-ticker as-of client wrapping one `CachedAsOfClient` per `TickerBundle`; shared `set_asof` fans out the ceiling, routes `get_*(ticker, ...)` per ticker, unknown ticker → empty/None (never raises). All routed signatures matched `CachedAsOfClient` as-is. TDD, test_asof_dispatcher.py 2 passed. Commit 955657d.
- T3 `asof_agent_context(dispatcher, scan_date)` (asof_agents.py) — context manager clamping ALL agent data paths: swaps `src.tools.api._v2_client_cache` + `v2.data.factory.get_provider_factory` (sector/macro agents bypass api.py), clears agent module dict caches `sector_agent._SECTOR_CACHE`/`_ETF_PRICES` + `macro_agent._CACHE` (real name is `_CACHE`, not `_MACRO_CACHE`), restores all on exit. `search_line_items` (yfinance) left as documented residual leak (A/B cancels common-mode). TDD, test_asof_agents.py 1 passed. Commit d70e1aa.
- T4 `build_bundles(tickers, provider_factory, start, end, *, enrich=True, deadline=None)` (bundles.py) — reuses `run_eval.prefetch_price_bundles` for price-only `{ticker: TickerBundle}`, then optional per-bundle `enrich_bundle` (earnings/insider/news/metrics) guarded by try/except. `enrich=False` stays fully offline. Signatures matched as-documented, no adaptation. TDD, test_bundles.py 1 passed. Commit b1a2103.
- T5 `scanner_arm` + `random_arm` (arms.py) — scanner_arm runs run_scan (injectable run_scan_fn seam) and returns (tickers, scanner_context); reused orchestrator `_entry_to_scanner_context` (importable, no side effects) wrapped under the ticker key, with inline fallback. random_arm: seeded `Random(f"{seed}:{scan_date}")`, deduped pool, reproducible per-date. TDD, test_arms.py 2 passed. Commit 4a33e0e.
- T6 `run_arm_decisions(*, arm, scan_date, tickers, scanner_context, model_name, model_provider, run_hedge_fund_fn=None)` (decisions.py) — runs `orchestrator.run_agents_only` (injected `run_hedge_fund_fn` stub keeps it offline; the stub's RuntimeError propagates out of run_agents_only since it calls the fn with no try/except), maps `out["decisions"]` → `{ticker: Decision}` reading action/quantity/confidence/reasoning via `.get()`; any exception → `ArmResult(error=...)`, decisions={}. Never raises. TDD, test_decisions.py 2 passed. Commit a989386.
- T7 `attach_forward_returns` + `ab_welch` (attribution.py) — attach_forward_returns rows BUY decisions only via reused `compute_forward_returns` (unclamped full-series fd so outcomes aren't clipped), row = {ticker, scan_date, action, confidence, **fwd_dict}. ab_welch: stdlib Welch two-sample t (ddof=1 sample variance), drops None entries, `diff=mean_a-mean_b`, `t=diff/sqrt(va/n_a+vb/n_b)`, t=None when n_a<2 or n_b<2 (means/diff still computed for n>=1). No scipy. TDD, test_attribution.py 4 passed. Commit 5946168.
- T8 `simulate(decisions_by_date, prices_by_ticker, *, trading_days, hold_days=21, starting_capital=100k, commission_bps=5, slippage_bps=5)` (portfolio.py) — equal-weight weekly-rebalance fixed-hold sim. Per trading day: exit positions entered `hold_days` ago (sell at close, cost on sell) → enter scan_date BUYs (equal cash split across priceable tickers, fractional shares, cost taken from the allocated slice so cash never goes negative) → mark all to close. value = cash + Σ(shares×close). Returns `{equity_curve:[{Date,"Portfolio Value"}], metrics, trades:[{ticker,entry_date,exit_date,entry_price,exit_price,shares}]}`; metrics via reused `PerformanceMetricsCalculator().compute_metrics(equity_curve)` (only "Date" + "Portfolio Value" keys load-bearing). TDD, test_portfolio.py 2 passed (full suite 16). Commit 38ff814.
- T9 `weekly_scan_dates` + `build_schedule` (regime_windows.py) — weekly_scan_dates: pure, filter trading_days to inclusive `[start,end]` then `[::every]` (indices 0,every,2*every,...). build_schedule: windows = `candidates or DEFAULT_CANDIDATES` + synthesized `{"name":"post_cutoff","start":post_cutoff_start,"end":run_date}`, classify ALL via reused `classify_regimes(spy_prices, all_windows)` (tolerates short constructed SPY list → degenerate CHOPPY, never raises), emit flat rows `{scan_date,regime_name,regime_label,is_post_cutoff=d>=post_cutoff_start}`. No dedup (windows non-overlapping). TDD, test_regime_windows.py 3 passed (full suite 19). Commit 29d9713.
- T10 `run_workflow_backtest` (run_workflow_backtest.py) + `report.write` (report.py) — integration capstone, fully DI so the smoke is OFFLINE. Per scan_date under `asof_agent_context`: scanner_arm→run_arm_decisions + random_arm→run_arm_decisions (sequential), collecting ArmResults + regime meta. RESUMABLE: each (scan_date,arm) cell persisted to `out_dir/cell_<date>_<arm>.json` and loaded back if present (verified: stubs that raise are NOT called on rerun). try/except per cell — one failure records an error, never aborts the run. Post-loop: `attach_forward_returns` per arm with UNCLAMPED `fd` (SPY benchmark fetched once via `fd.get_prices`), group BUY `ret_21d` by (regime,arm), `ab_welch(scanner_buys, random_buys)` per regime, equal-weight fixed-hold `simulate` per arm on post-cutoff dates (prices_by_ticker from bundle bars, trading_days = full sorted price-date set so holds can elapse). report.write → `findings_agent_backtest.md` (## A/B by regime + ## Absolute (post-cutoff)) + `decisions.csv`. Summary keys: `n_dates, arms_ran[date]→[arms], ab_by_regime, absolute_post_cutoff, report_path, decisions_csv`. CLI `main()` (argparse, `--smoke` = offline marker / no paid run; live path lazily binds `v2.scanner.runner.run_scan` + `src.main.run_hedge_fund` + `v2.data.factory.get_provider_factory()` — all import-verified). TDD, test_runner_smoke.py 1 passed (full suite 20). Commit 000cc13.
- T11 directional long+short (DESIGN GAP fix: framework only traded BUYs → bearish-day A/B n=0, 0 trades). A bet = `action∈{buy,short}` (buy→side+1, short→side-1; hold/sell/cover excluded). attach_forward_returns now includes shorts + adds `side` and `signal_ret_{N}d = side*ret_{N}d` (correct short on a faller scores positive). simulate enters buy+short, P&L-based equity `start + realized + Σ(side*shares*(price-entry))`, costs on both legs, trade rows gain `side`; fed compute_metrics a datetime-Date copy (fixes latent `.strftime` crash on any real drawdown). Runner groups direction-adjusted `signal_ret_21d` over ALL bets (`bets_by_regime_arm`) for the A/B; portfolio includes `action in (buy,short)`. report wording → "directional bets (long+short)" (A/B heading kept). TDD: +3 attribution (short-faller→+signal_ret, buy→raw, hold/sell/cover excluded) +3 portfolio (short-faller nets gain, short-riser loses, hold/sell not entered). Full suite 26 passed. Commit 82337e5.

## Session — workflow_backtest (launch + 2 pre-launch fixes)

- FIX report.py max_drawdown double-×100: `compute_metrics` already returns DD pre-multiplied (×100); report `_fmt(..., pct=True)` multiplied again → showed -1214.71% instead of -12.15%. Added `_pct_raw` helper (no ×100) for the already-percent DD cell. Commit 75e9173.
- FIX run_workflow_backtest CLI .env: live `main()` fetched SPY for `build_schedule` BEFORE `_live_seams()` (which loads .env transitively via `src.main`) → provider 401'd → 0 trading days → 0 scan dates (silent empty run). Added explicit `load_dotenv()` at top of the live path. Verified: schedule now builds 170 scan dates (bear_2022 40 / bull_2023_24 36 / choppy_2025 23 / post_cutoff 71; regimes classify BEAR/BULL/CHOPPY/BULL correctly). Commit 164bcab.
- Throwaway live pre-flight (1 date×3 tickers, real DeepSeek) had caught the BUY-only design gap (→ T11) and validated the live pipe end-to-end (NVDA short +3.42%). Deleted `_preflight_backtest.py` + `workflow_backtest_preflight/` after.
- LAUNCHED full paid run (background, ID bov3awxqb): `python -m v2.workflow_backtest.run_workflow_backtest --universe nasdaq100 --top-n 5 --every 5 --model deepseek-v4-pro --provider DeepSeek --hold-days 21 --seed 42 --out-dir workflow_backtest`. 170 dates × 2 arms = up to 340 cells, sequential-by-date (as-of monkeypatch is GLOBAL module state → must NOT date-parallelize), 13 agents run in parallel inside each `run_hedge_fund`. Resumable (`cell_<date>_<arm>.json`, existing skipped — re-launch same cmd to resume). Log: `workflow_backtest_run.log`. First cell (2022-01-03 scanner) wrote clean (error=None, 5 decisions). Report writes only on full completion → `workflow_backtest/findings_agent_backtest.md` + `decisions.csv`. ~7h est.
- BLOCKER — DeepSeek balance depleted mid-run (`402 Insufficient Balance`). Single run reached ~38/170 dates (bear_2022, to 2022-10-05) before the account ran out of credit; subsequent LLM calls failed 3×→`create_default_response` (all-hold conf0) = contaminated cells. (217 LLM-fail lines in run.log.)
- Parallel-sharding speedup (`_shard_backtest.py`, round-robin `schedule[i::N]`, multi-PROCESS so the global as-of monkeypatch can't collide; cells disjoint by date→no key collision) was LAUNCHED (2 canary shards) to cut ~16h→~4-5h — verified mechanically sound (cells writing, no rate-limit/concurrency issue) but immediately surfaced the 402. Killed both shards.
- CLEANUP: classified all cells (clean = has buy/short OR nonzero confidence; contaminated = all-hold+conf0). Deleted 14 contaminated + the boundary date 2022-10-05 (possible partial-contamination at the depletion edge). KEPT 74 fully-clean cells = 37 dates 2022-01-10→2022-09-28 (all directional-verified). User chose: top up DeepSeek + parallel-resume on the SAME model (deepseek-v4-pro, so A/B stays model-consistent). AWAITING top-up; will verify balance with a cheap 1-call test BEFORE relaunching shards (avoid re-contaminating). Then aggregate full report via one final full-schedule pass (loads all cached cells, no LLM).
- RESUME (3 parallel shards, `_shard_backtest.py --shard i --num-shards 3`, round-robin) ran clean to completion: 0×402 (balance held), ~1.28 min/cell (~2.6× speedup vs single). Effective coverage = 296 unique date+arm cells (342 schedule rows − 46 from choppy_2025∩post_cutoff date overlap; cells keyed by date+arm dedupe them).
- COMPLEX BUG pinned + fixed. Symptom: ~21 cells (all 2025-2026 / post-cutoff) errored `TypeError: '<' not supported between float and complex`, empty decisions (one ticker's crash propagates out of run_agents_only → whole cell lost). Repro (`_repro_complex.py`, 2025-01-02 random tickers CHTR/TRI/TSLA/INTC/NXPI) gave the traceback: `warren_buffett.py:549` `min(historical_growth, 0.15)` where `historical_growth` is COMPLEX. Root cause line 546: guard was `if oldest_earnings > 0:` — only checked the DENOMINATOR. INTC swung profit→loss (2024 negative net income), so `latest/oldest < 0`, and `negative ** (1/years)` = complex (Python). Fix: require BOTH endpoints positive (`oldest_earnings > 0 and latest_earnings > 0`) — mirrors the already-correct book-value CAGR guard at line 683; else falls to the existing 0.03 default. This is a PRODUCTION bug (crashes Buffett agent on any profit→loss company), not backtest-only. Commit 4c87a9f. Re-repro confirmed NO CRASH (INTC→buy). Deleted the 21 error cells; launched the full-schedule recompute+aggregate (loads 275 cached, recomputes 21 with the fix, writes report).

## Session — Analyze report upgrades (charts + grounding)

- USER REFRAME (load-bearing): the Analyze agent is NOT an auto-trader — it's a research tool to help a human understand a company + its current valuation FAST; the deliverable is the REPORT. Auto-trading profit is Lab's job. So evaluate the agent by report quality, not P&L. ("Understanding a company" ≠ "predicting its 21d price" — explains why agent direction is ~coin-flip yet the report is strong.)
- ② GROUNDING (anti-hallucination) — DONE + verified. Read the pipeline: `_llm_runner`/`executive_summary` already inject `build_quant_context` + a strong `QUANT_CONTEXT_DIRECTIVE` ("every number must come from the block"). Gap: the block surfaced only P/E·P/B·P/S + margins/growth, so the LLM INVENTED PEG/EV-multiples/FCF + alt-data (Firefly ARR, MAU, 13F, "underperformed IGV") from training memory. FIX (quant_context.py, commit 81ce174): surfaced the already-existing FinancialMetrics fields the LLM was inventing (enterprise_value, EV/EBITDA, EV/Rev, PEG, FCF yield/per-share, operating_margin, ROA, ROIC, current/quick ratio, interest_coverage); added REAL relative-perf vs sector ETF + SPY (from shared.sector_etf_prices/spy_prices); bumped news 5→12; extended the directive with rules 6-7 (non-numeric facts — product metrics/13F/M&A/exec changes — ONLY from the provided NEWS, never memory; relative-perf only from the block). Live ADBE report now cites PEG×5, EV/EBITDA×4, FCF×15, relative/sector×17 — all traceable.
- ① CHARTS — partial (data-limited). Subagent (opus) added 3 best-effort renderers to charts/render.py (commit 23592fd): render_fundamental_trends_png, render_valuation_band_png, render_relative_strength_png (+ 20 offline tests). Wired into the report via a generic `_section_chart_imgs` (html_render.py) reading `structured["charts"]`, + a guarded `_attach_chart` in sop_orchestrator.py attaching to financial_statements/valuation/sector sections (commit 10b3e5e). Live ADBE report went 2→ charts incl. the NEW relative-strength (stock vs sector ETF) ✓.
- DATA LIMITATION (why 2 of 3 charts don't show): hybrid provider gives only 1 period of fundamentals — EODHD basic returns [] for get_financial_metrics (/fundamentals 403), Finnhub gives a single current snapshot. So fundamental-trends + valuation-band (need ≥2 periods of history) skip cleanly. They'd light up with a historical-fundamentals source (yfinance is free + already used for search_line_items; or FD with a working key). Relative-strength works because prices have 276 bars.
- RESOLVED the data limitation (commit b5e67fb): new `src/research/charts/fundamentals_fetch.py` reuses `src.tools.line_items.search_line_items` (yfinance annual income statements) to build multi-period margins + revenue-growth, and computes a best-effort historical P/E (yfinance monthly closes ÷ annual EPS). sop_orchestrator falls back to this when `shared.financials < 2` periods. Live ADBE: 4 annual periods, P/E history 51.5→41.5→19.1 (shows the valuation de-rate). Regenerated report now has ALL 3 new charts (img 2→7): Relative strength + Fundamental trends + Valuation band. +4 offline tests (mocked search_line_items + yfinance). sop suite 33 passed, no network hang.

## Session — Paper-trading forward-test harness (overnight subagent build)

- Design + plan: spec `docs/superpowers/specs/2026-06-08-paper-trading-forward-test-design.md` (0dc3970), plan `docs/superpowers/plans/2026-06-08-paper-trading-forward-test.md` (a9ecf0c). Scope: zero-risk live PAPER trading on Alpaca, 3-sleeve live A/B (scanner_agent / scanner_only / spy_benchmark), long-only (backtest showed shorts are the leak), weekly scan + 21d hold, graduation bar before any real money. User reframe that grounds it: the Analyze agent is research-not-trading; auto-trading profit is a SEPARATE thing to be PROVEN on paper first (no LLM-foreknowledge, no lookahead — the rigorous instrument the backtest couldn't be).
- T1 broker (7059e1b): `BrokerClient` Protocol + deterministic in-memory `FakeBroker(starting_cash, prices)` (mutable prices = movable marks; never negative cash; oversell/unknown = noop; order-result dict {status,symbol,side,qty,price,reason}); `AlpacaBroker` stub raises NotImplementedError (no alpaca import). 11 tests.
- T2 models+migration (7dd5575): 4 SQLite tables PaperSleeve/PaperPosition/PaperOrder/PaperEquityMark + migration f0a1b2c3d4e5 (down_revision c5d2f0a1e9b7 — the TRUE head; impl caught my wrong mtime-based hint via alembic get_heads()). UniqueConstraint(sleeve_id,date) on equity marks. 5 tests. **alembic-migration-reviewer APPROVED** all 5 (chaining/symmetry/SQLite/additive/parity) + clean upgrade→downgrade→upgrade dry-run.
- T3 sleeves (6a8d047): `compute_targets(sleeve_name, scan_date, *, run_scan_fn, agent_fn, top_n)` over INJECTED seams — scanner_agent keeps action=="buy", scanner_only all picks, spy_benchmark ["SPY"]; never raises. `SLEEVE_NAMES`. 14 tests. Suite 30 passed.
- T8 FINAL integration (92c0c0a): ties the harness into a runnable system. **The DB is the source of truth** — the in-memory broker doesn't persist, so each run RECONSTRUCTS a per-sleeve FakeBroker from the DB and marks at REAL prices (3 sleeves can't share one Alpaca account; default path needs no broker key). (A) `FakeBroker.load_state(cash, positions)` seeds cash + open lots directly (copies the dict, no fill-replay → preserves cost basis) +3 broker tests. (B) `AlpacaBroker` filled — wraps alpaca-py `TradingClient(paper=True)`, keys from `ALPACA_API_KEY`/`ALPACA_SECRET_KEY`, **alpaca imported LAZILY inside `__init__`** (offline suite never imports it; clear ValueError on missing keys); all 5 BrokerClient methods over the SDK; NOT unit-tested live, only that it stays import-guarded. (C) `src/paper_trading/state.py::reconstruct_broker(sleeve_name, session, *, prices, starting_cash=100k)` — get-or-create sleeve → `derive_cash` → open PaperPositions (avg_price=entry_price) → `FakeBroker(...).load_state(...)`. (D) `src/paper_trading/run.py` CLI (`load_dotenv()` at top — lesson: prior CLI 401'd without it): `--once` (this week, all 3 sleeves), `--marks` (daily MTM), `--report` (md+html), `--scan-date`/`--week-key`/`--out-dir`/`--universe`/`--top-n`/`--model`/`--provider`/`--hold-days`. Live seams lazily bound (`--report` alone needs NO providers): run_scan_fn mirrors `workflow_backtest/arms.scanner_arm` (`[e.ticker for e in run_scan(...)]`), agent_fn mirrors `decisions.run_arm_decisions` (`run_agents_only(...)["decisions"]`), price_fn mirrors `shared_data` (provider_factory → `get_prices(t,start,asof)[-1].close`, best-effort None). `run_once` isolates per-sleeve failures (one bad sleeve never sinks the run). Session via `app.backend.database.SessionLocal`. Defaults mirror workflow CLI (nasdaq100 / deepseek-v4-pro / DeepSeek / top_n 5 / hold 30). (E) Scheduler WIRED (not just exposed): zero-arg `paper_weekly_job()` + `paper_daily_marks_job()` in run.py, registered as singleton crons in `SchedulerService.start()` via new `_register_paper_trading_jobs` (Mon 17:00 ET rebalance `paper_weekly`, weekday 17:05 ET marks `paper_daily_marks`; bodies lazy-imported so scheduler import doesn't drag in the scanner/LLM stack; both idempotent → retry-safe). Updated 2 add_job count assertions in test_scheduler_service.py (5→7, 3→5). (F) `src/paper_trading/test_smoke.py` OFFLINE e2e: scratch SQLite + reconstruct_broker + FakeBroker + stub seams → run_week ×3 sleeves → mark_all → compute_performance + evaluate_graduation → write_report(tmp_path); asserts positions+orders+equity marks materialized, report files exist, re-run idempotent (already_ran, 0 new orders), and **`"alpaca" not in sys.modules`**. alpaca-py pinned as OPTIONAL `[alpaca]` extra in pyproject (^0.42.0; deps requests/pydantic/pandas/msgpack/websockets/sseclient-py all compatible → resolves clean in poetry lock, unlike mootdx; not in slim prod image). black-clean (420). New tests: 1 smoke + 3 broker = 4. Full src/paper_trading/ suite 68 passed; tests/test_scheduler_service.py 20 passed; other scheduler suites (research/screener/preset) 11 passed.

### Paper-trading harness — BUILD COMPLETE (T4-T8 + review)
- T4 engine c38a88d (8t), T5 marks 025fa35 (7t), T6 performance 338f43a (12t), T7 report 1a3c975 (7t), T8 alpaca+CLI+scheduler+smoke 92c0c0a (4t). DB is source of truth; each run reconstructs a per-sleeve FakeBroker from the DB (derive_cash + open positions) at REAL prices — the 3-sleeve A/B needs NO broker key (alpaca-py is an OPTIONAL extra; AlpacaBroker is a future real-fills adapter, lazy-imported). Scheduler WIRED: paper_weekly (Mon 17:00 ET), paper_daily_marks (weekdays 17:05 ET).
- FINAL holistic review (opus): state round-trip CLEAN (reconstructed cash+positions exactly match engine writes — fills persisted at the broker's actual qty×price). Found + FIXED (commit 7a68f9a, +2 regression tests, suite 70 passed): (CRITICAL) run_once passed one hold_days to all sleeves → spy_benchmark churned every 30d instead of buy-and-hold → now hold_days=None for spy; (MEDIUM) equal-weight under-deployed when a target was unpriceable → now splits cash across priceable targets only. Deferred LOW: zero-order weeks not strictly idempotent; AlpacaBroker could log a filled order at price=0.0 (unused path) — fix before any live Alpaca use.
- STATUS: offline build done, 70 tests green. To START live paper trading: nothing to install (default uses internal ledgers + real price data the app already has). Run `python -m src.paper_trading.run --once` weekly + `--marks` daily (or let the wired scheduler fire), then `--report`. Graduation bar (before ANY real money): ≥3 months, scanner_agent positive + Sharpe≥SPY + maxDD<20% + return≥scanner_only.

## Session — Institutional-flow signals (gamma + short-volume) into the agent
- User wanted dark-pool + gamma to track institutions (US market is institution-dominated). Built the FREE version (verified data works), honest that true ATS dark-pool needs paid (Unusual Whales/SpotGamma).
- ① GAMMA/GEX (commit 09e1a88): src/research/institutional_flow.py — `fetch_gamma_exposure(ticker)` computes dealer GEX from yfinance options chains (free): BS gamma × OI, SqueezeMetrics sign (calls−puts), returns total_gex/regime(pos=pinned,neg=squeeze-prone)/gamma_flip/walls. 20 tests. Live verified: SPY −$3.5B NEGATIVE, NVDA +$425M POSITIVE.
- ① AGENT INTEGRATION (commit b7c899a): quant_context.py injects a gatable+cached "INSTITUTIONAL POSITIONING — DEALER GAMMA" block. Flag `set_flow_enabled(bool)`/`flow_enabled()` (DEFAULT True — normal Analyze uses it). 6 tests.
- ② SHORT-VOLUME (commit 67222cf): `fetch_short_volume(ticker)` from FINRA Reg-SHO daily CDN `https://cdn.finra.org/equity/regsho/daily/CNMSshvol{YYYYMMDD}.txt` (FREE, no-auth, User-Agent header, weekends 403, aggregate multi-Market rows) → short_pct + trend. Honestly labeled a PROXY (not true ATS). Rendered after gamma in the same block. 29 tests. Live: NVDA 34%, GME 61%.
- ③ 4th SLEEVE (commit b910e82): `scanner_agent_flow` added to SLEEVE_NAMES; compute_targets treats it like scanner_agent; run_once toggles `set_flow_enabled(name=="scanner_agent_flow")` per sleeve (restored True in finally). Clean A/B: scanner_agent (flow OFF) vs scanner_agent_flow (flow ON). Agent runs 2× per week now. 18 tests, paper_trading 75 passed; research+paper combined 148 passed.
- STATUS: all live-verified; agent now SEES institutional gamma + short-pressure when analyzing any ticker. The paper harness's 4th sleeve will forward-test whether flow-awareness adds value. NOT YET in the DB (existing 3 sleeves started 2026-06-08; a reset + re-run --once would start all 4 aligned).

## Session — Surface institutional flow to the human (report section + frontend panel)
- ① REPORT SECTION (commit f62c6c0, opus subagent): new non-LLM "Institutional Positioning" report section (`src/research/sections/institutional_flow.py`) — renders dealer-gamma regime/GEX/flip/walls + short% + a gamma-walls chart (`render_gamma_walls_png`). Wired into SECTION_ORDER (after technical) + _HEADING_MAP + zh heading "机构持仓" + template + _PARALLEL_SECTIONS. Fixed the pre-existing section-count test (was 16-expected-but-17-real omitting catalyst; now 18 incl. institutional_flow). 10 tests; research suite 297 passed.
- ② BACKEND ROUTE (commit cf101dc): `GET /institutional-flow/{ticker}` → `{ticker, gamma, short_volume}` (best-effort, nulls not 500). 3 offline tests. Registered in api_router.
- ② FRONTEND PANEL (commit 8b8e906, built by me — subagent kept hitting transient rate limits): `institutional-flow-service.ts` + `InstitutionalFlowPanel` (ticker input → gamma regime color-coded + net GEX + flip + walls table + short% + trend; honest "proxy not true dark-pool" labels). Wired: TabType 'flow', TabService.createInstitutionalFlowTab, sidebar FlowAction (Activity icon, after Analyze), tab-bar icon, en+zh i18n. tsc --noEmit = 0 errors.
- ③ forward-test continues (4 sleeves aligned 2026-06-08; accrues on weekly --once; deploy Fly for unattended). To SEE ②: run backend (uvicorn :8001) + frontend (npm run dev), open the "Institutional" sidebar tab.

## Session — Self-evolving factor-strategy engine (v2/self_evolve/, overnight subagent build)

- Premise (Huatai AutoResearch / Karpathy self-evolving Skill): an LLM proposes single-hypothesis CONFIG deltas to a DETERMINISTIC factor strategy, under train/val/test sample isolation + version management; best val-retained version graduates to the paper forward-test. The methodology is process discipline, NOT edge creation — the val→test→forward-test gauntlet is the guard. Spec 5b87e18, plan a825958. Medium-term (1-3mo, monthly rebal), price-led factors + shallow value/quality, lives in Lab.
- T1 config (b3f6c93): StrategyConfig (factor_weights momentum/low_vol/reversal/value/quality sum-normalized + lookbacks + top_n[20,50] + max_weight[3-8%] + liquidity_pct + rebalance + cost_bps); ADJUSTABLE (min,max) ranges = the protocol boundaries; load_config/validate/apply_delta (ConfigError on out-of-range). + strategy_skill/SKILL.md + skill_config.yaml baseline. 13 tests.
- T2 samples (a513190): immutable train(2016-2021)/val(2022-2023)/test(2024+) split; sample_of; rebalance_dates (first trading day each month). 13 tests.
- T3 factors (d6c4152): compute_factors NO-LOOKAHEAD — _asof_closes drops bars>asof; momentum(12-1)/low_vol/reversal end ≤asof; value(E/P)+quality(ROE) from _latest_lagged_metric (cutoff asof−60d). 10 tests.
- T4 strategy_gen (0d0e257): generate_holdings — liquidity filter, cross-sectional z-score (STD_FLOOR 1e-9, None→neutral), composite via factor_weights, top_n, inverse-vol weights, iterative cap+renormalize at max_weight. 12 tests.
- T5 backtest (ae4adbe): backtest(bundles,config,sample) → {sharpe,ann_return,ann_vol,max_drawdown,turnover,n_rebalances}; monthly equity (datetime Dates, 12/yr), geometric CAGR, PerformanceMetricsCalculator (max_drawdown AS-IS, no double ×100), turnover = mean 0.5Σ|Δw|. 7 tests.
- T6 versioning+proposer (c63ef11): write/read_version + append/read_path_log + list_versions (best-effort, never crash); propose(skill_md,config,val_history,*,llm_fn=None) → one validated {path,value,hypothesis}|None (validated against ADJUSTABLE via apply_delta; bad JSON/out-of-range/llm raise → None; default llm_fn lazily binds DeepSeek). 13 tests.
- T7 loop (f875f48): evolve(bundles,base_config,*,iterations,base_dir,skill_md,propose_fn,backtest_fn) → path_log. Keep iff val Sharpe strictly improves AND guardrails (turnover≤base×1.5, val maxDD not worse by >5pp) else rollback. TEST NEVER backtested in the loop (only "train"/"val" literals; sample-recorder test asserts "test" never seen). Resumable from last KEPT version. 10 tests.
- T8 report+CLI+smoke (816da8a): render_iteration_path_png + write_report (iteration-path table + PNG + retained config + TEST verdict + honest "val≠edge" caveat); run.py CLI (--iterations/--universe/--base-dir/--out-dir, load_dotenv, providers lazy) reads TEST exactly ONCE post-loop; offline e2e smoke (synthetic bundles + stub proposer; asserts "test" not in loop samples + "src.llm.models" not imported). 2 tests.
- T9 graduate (82df69f): load_best_config (last KEPT version, fallback baseline) + build_factor_fn live seam; compute_targets gains "factor_evolved" sleeve (injected factor_fn, never-raises); threaded through run_once + engine.run_week; SLEEVE_NAMES now 5. No-lookahead live: bundles end=scan_date, generate_holdings asof=scan_date STRING (avoided the _parse_iso asof[:10] date-object→{} trap). 10 tests.
- FINAL review (opus, read-only): GREEN on the two load-bearing invariants — no-lookahead holds end-to-end (price-layer clamp + 60d fundamental lag + forward-only returns, verified empirically with a post-window price spike) and TEST never read in the loop (literal call sites + single post-loop read + sample-recorder tests). std-floor + never-raise + metrics conventions all correct. Found HIGH/MEDIUM realism gaps (not safety): H1 inert ADJUSTABLE knobs, H2 gross (no-cost) returns, M1 silent empty book if asof is a date.
- FIXES (9672063): H2 charge transaction costs (net returns: cost_i = Σ|Δw_i| × cost_bps/1e4, compounded net) so the loop optimizes NET not gross; H1 drop tilt_strength + holding_buffer from ADJUSTABLE (read nowhere — intended future levers, not yet wired — so the proposer can't waste paid iterations) + add cost_bps as a real lever now; M1 logger.warning naming the type before returning {} on unparseable asof. self_evolve 99 passed, paper_trading 75 passed.
- STATUS: build COMPLETE (T1-T9 + review + fixes), 99 offline tests green, no regressions. To RUN an evolution: python -m v2.self_evolve.run --iterations N --universe nasdaq100 --out-dir <dir> (live DeepSeek proposer; reads TEST once at the end). Best version graduates via the factor_evolved paper sleeve (forward-test alongside the other 4 sleeves). DEFERRED (need a design decision, noted for the user): wire tilt_strength (value/quality tilt scaler) + holding_buffer (rank hysteresis — needs stateful rebalancing); M2 mktcap-proxy=advol duplicate; a Lab panel to launch/watch runs (fast-follow).
## Session — Deploy paper forward-test unattended (sub-project A, code portion)

- Context: the 5-sleeve paper forward-test was frozen at 1 equity mark (2026-06-08) because it only ran on manual CLI invocation. Goal: run the 4 LIGHT sleeves unattended on the live Fly app (scheduler + persisted SQLite already wired); exclude the heavy factor_evolved sleeve (its live factor_fn rebuilds a full nasdaq100 ×3yr enrich bundle = ~30min/OOM risk on a 2GB prod box; factor strategy is monthly anyway) until sub-project B+C speeds it. Spec 7604b2a, plan 9b13fd2.
- T1 active_sleeves() (c483961): src/paper_trading/sleeves.py gains active_sleeves() reading PAPER_SLEEVES env (comma-sep). Unset/blank/all-unknown -> all SLEEVE_NAMES (local unchanged); known subset -> that subset in SLEEVE_NAMES order; unknown names dropped with a warning; never raises. 7 tests (test_sleeves.py 16->23).
- T2 run_once gate (a573357): run.py run_once iterates active_sleeves() instead of SLEEVE_NAMES (the only iteration site; mark_all already iterates DB rows so an un-seeded sleeve is never marked). fly.toml [env] sets PAPER_SLEEVES="scanner_agent,scanner_only,spy_benchmark,scanner_agent_flow" (factor_evolved excluded on prod). New offline test (scratch SQLite + stub seams): PAPER_SLEEVES=2-subset -> run_once seeds/summarizes only those 2. paper_trading suite 75->83.
- T3 pre-flight (no commit): paper_trading + self_evolve = 182 passed; verified the scheduler paper-job import path (src.paper_trading.run paper_weekly_job/paper_daily_marks_job) drags NO alpaca (ALPACA MODULES: []); confirmed scheduler registers paper_weekly (0 17 * * 1) + paper_daily_marks (5 17 * * 1-5) in America/New_York.
- STATUS: code done (Tasks 1-3). Remaining Tasks 4-10 are USER-RUN outward fly/git (push origin -> verify secrets -> volume snapshot -> fly deploy -> verify boot logs -> ssh --once --marks to seed -> --report to verify; rollback via fly releases rollback + the snapshot). The deploy is verified live by runbook criteria, not the test suite.
## Session — Sub-project A DEPLOYED (paper forward-test live on prod, unattended)

- Tasks 4-10 (USER-RUN outward, executed via authed flyctl at C:\Users\Jerry\.fly\bin\fly.exe as ruizheyuan3487@gmail.com):
  - PUSH: 23+ commits -> origin/main (da48ecf).
  - SECRETS: fly secrets list showed all present EXCEPT DEEPSEEK_API_KEY (missing). Staged it from local .env via `fly secrets set --stage` using a runtime variable (value NEVER echoed; first attempts hit a UTF-8 BOM on the .env first line + a PS-stdin BOM injection -> switched to passing "DEEPSEEK_API_KEY=$val" as an arg with the literal key name). Other keys (EODHD/FINNHUB/FINANCIAL_DATASETS/JWT/APP_ENCRYPTION/RESEND) already Deployed.
  - SNAPSHOT: fly volumes snapshots create vol_vde6lnxynm0993k4 (pre-deploy DB insurance; + Fly daily auto-snapshots).
  - DEPLOY: fly deploy SUCCESS. Boot logs confirm: `Running upgrade c5d2f0a1e9b7 -> f0a1b2c3d4e5, add_paper_trading_tables` (migration applied to /data/app.db) + `Registered paper-trading jobs paper_weekly (0 17 * * 1) + paper_daily_marks (5 17 * * 1-5), tz=America/New_York` + `Application startup complete`. /health=200. (The transient "not listening" warning was the ~20s alembic-migration window before uvicorn bound.)
  - SEED: `fly ssh -C "python -m src.paper_trading.run --once --marks"` -> 4 sleeves opened + marked at $100k, factor_evolved ABSENT (PAPER_SLEEVES gate works on prod): scanner_agent entered=[] (agent no buy this week), scanner_only=[ODFL,TTWO,PCAR,HON,LIN], spy_benchmark=[SPY], scanner_agent_flow entered=[] (flow agent also no buy). `marked 4 sleeves`. (The trailing "Error: handle is invalid" + exit 1 is a benign fly-ssh-on-Windows teardown artifact AFTER the work completed.)
  - REPORT: --report passed=False (correct for day 1; graduation needs >=3mo + beat SPY).
- STATUS: A COMPLETE. The forward-test now runs UNATTENDED on prod: weekly rebalance Mon 17:00 ET + daily marks weekdays 17:05 ET, accruing for ~3 months. factor_evolved joins after B+C speeds its bundle build.
- NEXT: B+C (sub-project #452) scope chosen by user = "speed-first + a few factors": factor-value cache (17min/iter -> seconds) as the main effort + 3-4 cheap BACKTESTABLE factors (volume momentum + price/fundamental). Constraint surfaced: the self-evolve loop backtests train/val, so C can only add point-in-time-historical factors; gamma/flow are forward-only (already in the paper scanner_agent_flow sleeve), NOT for the loop.
## Session — Sub-project B+C Part B: factor cache + parse memoization (DONE)

- Spec 729727d (B cache + C 11 factors + line-items enrich); Part B plan 9b13fd2... (2026-06-10-self-evolve-cache-partB.md). Part B = the speed win ONLY; Part C (line-items enrich + 11 factors) is a separate later plan.
- T1 (825e158-chain): compute_factors(bundles, asof, config, *, cache=None) wraps the _compute_one call; key=(ticker, asof_iso, _lookback_cache_key(config)=(momentum_days,vol_days,reversal_days)). cache=None default => existing callers unchanged. No-lookahead preserved by construction (asof in key). 3 tests (cached==fresh, pure-hit zero-recompute, per-lookback invalidation==fresh).
- T2: generate_holdings(..., *, cache=None) forwards to compute_factors. T3: backtest(..., *, cache=None) forwards to generate_holdings per rebalance. T4: evolve binds ONE shared cache via a nested def ONLY when backtest_fn is None (injected stubs keep their (b,c,s) arity untouched -> existing loop tests unaffected). 6 cache tests total.
- PROFILING FINDING: factor cache alone only ~2x, because the LIQUIDITY filter (_asof_bars) re-parses every bar's date every rebalance -> _parse_iso called 128,008 times = 6.4s of 8s. Fix: @lru_cache(maxsize=None) on _parse_iso (pure str->str|None, transparent). 
- RESULT (20-ticker demo): cold backtest 6.61s->0.78s (8.5x), warm 3.79s->0.38s (17x); full v2/self_evolve/ suite 102s->22s (4.5x). 105 tests pass. The real nasdaq100 17min/iteration should drop dramatically (verify with a live run before Part C).
- Commits: factor cache (compute_factors/generate_holdings/backtest/evolve, 4 commits) + perf(_parse_iso memoize) 825e158. 
- NEXT: optionally confirm the speedup on a real (small) nasdaq run, then Part C (line-items enrich + the 8 new/fixed factors: MAX/52w/resid_mom/turnover + real value/gross_prof/asset_growth/ROE).
- Part C TASK 12 (final integration, Part C COMPLETE): balanced the 11-factor baseline `factor_weights` in strategy_skill/skill_config.yaml off the token 0.05 seeds to a defensible price-led book — momentum .16 / reversal .12 / low_vol .12 / resid_mom .10 / high_52w .08 / max_lottery .08 / turnover .06 (price/vol ~72%) + value .10 / gross_prof .06 / asset_growth .06 / quality .06 (fundamentals ~28%); raw sum 1.0, normalizes to 1.0, momentum leads. Lookbacks unchanged (max 21/hi 252/to 21/resid 252).
- T12 SKILL.md: documented the full factor kernel — 11-factor catalog (definition + anchor paper each: JT momentum, Jegadeesh reversal, Ang IVOL, Blitz-Huij-Martens resid-mom, George-Hwang 52w, Bali-Cakici-Whitelaw MAX, Datar-Naik-Radcliffe turnover, FF value/HML, Novy-Marx gross-prof, Cooper-Gulen-Schill/FF5-CMA asset-growth, ROE quality) + no-lookahead discipline (prices ≤ asof, fundamentals lagged 60d) + proposer adjusts ADJUSTABLE only. Refreshed the knob table to all 11 weights + 7 lookbacks + cost_bps (removed holding_buffer/tilt_strength rows — NOT actually in ADJUSTABLE per config H1).
- T12 smoke test (appended to v2/self_evolve/test_factors_partc.py, +3 tests): (1) baseline loads+validates with 11 normalized weights, min>0.05, momentum=max; (2) compute_factors on a 40-ticker bundle w/ line_items_history emits all 11 keys non-None (resid_mom real cross-sectionally); (3) generate_holdings with factor_weights.value up-weighted to 1.0 yields a DIFFERENT book than baseline (disjoint held set, mean held-index strictly lower) — proving the new factors drive selection. Universe uses a shared ±2% market path + per-ticker constant drift so low_vol z≈0 (no confound) and momentum(high-i) vs value(low-i) pull opposite.
- T12 VERIFY: full v2/self_evolve/ suite 163 passed (was 160 + 3 new); ZERO existing tests adjusted — all assert structural invariants (sum=1.0 / key set / ranges / post-delta ordering) or hand-built configs, none pinned an exact composite tied to the old weights. validate(load_config(...)) OK. black clean.
## Session — Sub-project B+C Part C: 11-factor library + line-items data path (DONE)

- Spec 729727d; Part C plan (2026-06-10-self-evolve-factors-partC.md), 12 tasks, executed subagent-driven (all opus).
- Phase 1 (data): `fetch_line_items_history` (yfinance via src.tools.api.search_line_items, best-effort, never-raises) attaches `bundle.line_items_history`; `_latest_lagged_line_item` + `_prior_year_line_item` (60d as-of lag) in factors.py. Commits 6313691, 0b6e2bb.
- Phase 2 (scaffold): registered 11 FACTOR_KEYS (config + factors.py mirror in sync) + ADJUSTABLE factor_weights.* + lookback.{max_days,hi_days,to_days,resid_days} + extended `_lookback_cache_key` with the 4 new windows + skill_config.yaml 11 weights. Test fixtures (test_loop/test_smoke _base_config) got the new keys at 0.0 (neutral, byte-identical). Commit 7c712aa.
- Phase 3 (factors): MAX/52w/turnover in _compute_one (99b8f73/98200f1/6b93a6f); value(E/P)/gross_prof/asset_growth/ROE rebuilt on line items + DELETED inert _value_from_metric/_quality_from_metric (60db368/43816a0/2248ab9/43501b6); resid_mom cross-sectional in compute_factors (OLS intercept = idiosyncratic alpha; cache-no-pollution via shallow copy) (90b5635).
- Phase 4 (integration): balanced 11-way weights (momentum 0.16 lead, price block ~72% / fundamentals ~28%), SKILL.md kernel doc, smoke (a value up-weight delta changes holdings → factors live). Commit f2e3316.
- FINAL review (opus, read-only): GREEN on (1) no-lookahead across all 11 factors (price clamp <=asof, fundamentals <=asof-60d, resid_mom both series clamped; post-asof-spike tests non-vacuous) and (2) resid_mom cache-no-pollution. FACTOR_KEYS sync / never-raise / inert-removed / _parse_iso-lru all PASS. 0 CRITICAL/HIGH.
- FIX M1 (82f148a): gross_prof was mixing gp/total_assets (line items) with gross_margin fallback (metrics) in ONE cross-sectional z-score = apples-to-oranges on mixed-coverage universes. Now a SINGLE homogeneous source: gross_margin only (reliably enrich-populated). Rewrote the gross_prof tests to the homogeneous contract.
- DEFERRED (review follow-ups, non-blocking): M2 silent-empty WARNING (a factor None for 100% of names) — belongs in the run/eval harness not the hot compute_factors (per-call check would false-positive); L1 resid_mom is residual ALPHA (intercept) not standardized Blitz residual-momentum (acceptable, doc note).
- STATUS: B+C COMPLETE. self_evolve suite 161 passed. To run the (now fast) live 11-factor evolve: `python -m v2.self_evolve.run --iterations N --universe nasdaq100 --out-dir <dir>`. Best version graduates to the paper factor_evolved sleeve (once added to PAPER_SLEEVES on prod).
## Fundamental report sections (capital_structure + ownership_structure) — 2026-06-11

- TASK 1 (ownership fetch): `src/research/ownership_fetch.py::fetch_ownership(ticker)->dict` — yfinance `.info` (heldPercentInsiders/heldPercentInstitutions/sharesOutstanding) + `.major_holders` (institutionsCount) + `.institutional_holders` (top rows → {name, pct}); lazy `import yfinance` inside the fn; single try/except → all-None dict (never raises). 5 offline tests (fake yfinance via sys.modules). Commit 01aa89b.
- TASK 2 (capital_structure section): `src/research/sections/capital_structure.py` + `prompts/modules/capital_structure.md`. `_capital_block(ctx)` fetches annual line items via `search_line_items` (patchable seam on the section module; wrapped try/except → []), applies a 60d availability lag (`_latest_lagged`/`_prior_year` local helpers), and renders GROUNDED ratios: debt/equity, net_debt, leverage, interest_coverage (omitted when no operating_income), cash, shares_outstanding (+YoY dilution). Each ratio None/zero-denominator guarded → "n/a". No data → "data unavailable" note (never raises). LLM narrates via run_llm_section with the block PREPENDED. 6 offline tests (LLM + search_line_items both stubbed). Commit 33a7582.
- TASK 3 (ownership_structure section): `src/research/sections/ownership_structure.py` + `prompts/modules/ownership_structure.md`. `_ownership_block(ctx)` calls `fetch_ownership(ticker)` (patchable seam; double-wrapped never-raise) + computes the insider net = signed Σ transaction_shares off `ctx.shared.insider_trades` (with direction net buying/selling). GROUNDED fields: insider %, institution %, institution count, top holders (name + %), shares outstanding, insider net. All None-guarded → "n/a". Both ownership-fetch AND insider-enrich empty → "data unavailable" note (never raises). LLM narrates via run_llm_section with the block PREPENDED. 6 offline tests. Commit 094c3d7.
- TASK 4 (register both): SECTION_ORDER (models.py) gains capital_structure (after financial_statements) + ownership_structure (after it) → 20 sections; _HEADING_MAP ("Capital Structure"/"Ownership Structure") + _HEADING_ZH_MAP ("资本结构"/"股权结构") in html_render.py; both added to _PARALLEL_SECTIONS in sop_orchestrator.py (LLM batch); section modules already self-register via sections/__init__.py imports. Updated test_models_phase4 → 20 + a new en/zh-heading test. Fixed two stale "10 _PARALLEL_SECTIONS" doc comments. FULL `tests/research/ src/research/` regression GREEN: 315 passed. Commit 9ae2e08.
- FRONTEND (deliberate, out of scope): mirrors institutional_flow which is ALSO backend-only — not on the default-template.ts canvas seed (PARALLEL_SECTIONS). New sections still run for direct/API/cron calls (included_sections defaults to all of SECTION_ORDER) and use-section-labels.ts falls back to the raw name. Frontend canvas/i18n labels are a separate change (frontend package.json/pnpm-lock already have unrelated uncommitted edits — left untouched).
- HANDOFF: (II) scanner fundamental signals + (III) scanner self-evolve remain specced for next session in docs/superpowers/specs/2026-06-11-fundamental-depth-and-scanner-design.md (ownership is report-only, no scanner). DEFERRED — not built this session.

## Scanner Self-Evolve — TASK 6: evolve wiring + CLI (DONE) — 2026-06-11

- KEY DECISION (user): inject BACKWARD-COMPATIBLE seams into the SHARED self_evolve engine rather than fork it. All seams default to the factor behavior → the entire v2/self_evolve/ suite stayed green untouched (161 passed; the 1 pre-existing test_graduate failure is unrelated — it reads the real 11-factor strategy_skill dir vs a hardcoded 5-factor expectation; confirmed failing WITHOUT my changes via git stash).
- Part A (v2/self_evolve/loop.py): added optional `apply_delta_fn` / `rebuild_config_fn` / `keep_fn` to `evolve(...)`. Refactored running-best state from scalar (best_sharpe,best_maxdd) → `best_val_metrics` dict + `base_val_metrics` (v0 baseline dict, anchors the turnover ceiling across resumes). `_keep`→`_default_keep(cand_val,best_val,base_val)` reproduces factor keep byte-for-byte. `_resume_state` now returns (start_round,cfg,best_val_metrics,base_val_metrics) via the injected rebuild. Broadened delta-apply except to (ValueError,KeyError,TypeError) — both factor & scanner ConfigError subclass ValueError. Dropped now-unused ConfigError import.
- Part B (v2/self_evolve/proposer.py): `propose(...)` gained `adjustable=None` (default factor ADJUSTABLE) + `apply_delta=None` (default factor apply_delta); used for the path check, _build_prompt, and the authoritative gate. Broadened `except ConfigError`→`except ValueError`. Factor callers passing neither = identical behavior.
- Part C (v2/scanner/evolve/run.py): SCANNER_SKILL_MD prose discipline (price-only SCREENER, event_weight=1.0/quant_weight=0.0 FIXED, objective = A/B-vs-random val `diff` not directional alpha). `_rebuild_scanner_config`, `_scanner_keep` (None-diff→reject; n_fired floor=max(5,0.5*base); diff strictly improves; t_stat not worse by >0.5). `evolve_scanner(...)` closes a run-scoped parse `cache` into a `scanner_fitness` backtest_fn + injects all 3 loop seams + the SCANNER_ADJUSTABLE/scanner_apply_delta proposer. CLI `main(argv, *, prefetch_fn, spy_fetch_fn)`: full prefetch span = union of train+val+TEST windows padded (400d/45d), prefetch ONCE, evolve (train+val only), then read TEST exactly once post-loop on the retained-best config, write inline JSON summary (Task 7 marker left for report.py).
- Tests (v2/scanner/evolve/test_run.py, all offline): wiring (v0+round versions written), _scanner_keep direct (kept / collapsed-n_fired rollback / abs-floor / lower-diff rollback / worse-t rollback / None-diff), keep+rollback via the loop, SAMPLE ISOLATION recorder ("test" never seen; train+val are → non-vacuous), resumable (counter continues + running-best restored, proposer sees threshold 4.0), factor-engine-untouched (no-seam evolve still keeps). Stub fitness injected by monkeypatching run_mod.scanner_fitness.
- VERIFY: v2/scanner/evolve/ 53 passed; v2/self_evolve/ 161 passed (1 pre-existing unrelated fail); v2/scanner/test_runner.py 15 passed. black clean on all 4 touched files. Commit 312b33d.
- NEXT: Task 7 — report.py (md/html) + refactor the CLI inline summary to use it + the CLI smoke test (injected prefetch, exercises the network path offline).
