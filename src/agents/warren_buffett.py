...
def calculate_intrinsic_value(financial_line_items: list) -> dict[str, any]:
    ...
    if len(historical_earnings) >= 3:
        ...
        # Both endpoints must be positive: a profit→loss swing makes the ratio
        # negative, and a negative base to a fractional power yields a COMPLEX
        # number (Python), which then crashes the min() comparison below.
        if oldest_earnings > 0 and latest_earnings > 0:
            ...
            conservative_growth = 0.03  # Default 3% if either endpoint is non-positive (CAGR undefined)
    else:
