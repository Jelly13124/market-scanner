...
class TestPipelineRunLifecycle:
    def test_create_pending_persists_basic_fields(self, runs):
        run = runs.create_pending(
            run_id="abc123", scan_date="2024-08-01", template="quick",
            selected_analysts=["scanner_signal", "fundamentals_analyst"],
            top_n=5, universe="nasdaq100",
        )
    ...
    def test_mark_running_sets_started_at(self, runs):
        runs.create_pending(run_id="r1", scan_date="2024-08-01",
                            template="quick", selected_analysts=["scanner_signal"],
                            top_n=1, universe="nasdaq100")
    ...
    def test_mark_complete_writes_blobs_and_duration(self, runs):
        runs.create_pending(run_id="r1", scan_date="2024-08-01",
                            template="quick", selected_analysts=["scanner_signal"],
                            top_n=2, universe="nasdaq100")
        runs.mark_running("r1")
        watchlist = [{"ticker": "AAPL", "rank": 1, "composite_score": 87.5}]
        decisions = {"AAPL": {"action": "buy", "quantity": 10}}
        signals = {"scanner_signal_agent": {"AAPL": {"signal": "bullish",
                                                       "confidence": 88,
                                                       "reasoning": "x"}}}
        updated = runs.mark_complete(
            "r1", watchlist=watchlist, agent_decisions=decisions,
            analyst_signals=signals, duration_seconds=12.5,
        )
...
class TestPipelineRunListing:
    def test_list_runs_newest_first(self, runs, db_session):
        ...
        for i, rid in enumerate(["a", "b", "c"]):
            runs.create_pending(run_id=rid, scan_date="2024-08-01",
                                template="quick",
                                selected_analysts=["scanner_signal"],
                                top_n=1, universe="nasdaq100")
            db_session.query(PipelineRun).filter(PipelineRun.id == rid).update(
    ...
    def test_filter_by_template(self, runs):
        for i, tpl in enumerate(["quick", "balanced", "quick"]):
            runs.create_pending(run_id=f"r{i}", scan_date="2024-08-01",
                                template=tpl,
                                selected_analysts=["scanner_signal"],
                                top_n=1, universe="nasdaq100")
    ...
    def test_filter_by_status(self, runs):
        runs.create_pending(run_id="r1", scan_date="2024-08-01",
                            template="quick", selected_analysts=[],
                            top_n=1, universe="nasdaq100")
        runs.create_pending(run_id="r2", scan_date="2024-08-01",
                            template="quick", selected_analysts=[],
                            top_n=1, universe="nasdaq100")
        runs.mark_complete("r1", watchlist=[], agent_decisions={},
                           analyst_signals={}, duration_seconds=1.0)
    ...
    def test_filter_by_since_scan_date(self, runs):
        for sd, rid in [("2024-07-01", "old"), ("2024-08-01", "new")]:
            runs.create_pending(run_id=rid, scan_date=sd, template="quick",
                                selected_analysts=[], top_n=1,
                                universe="nasdaq100")
...
class TestPipelineSchedule:
    def test_get_returns_seeded_singleton(self, schedule):
        row = schedule.get()
    ...
    def test_partial_update_only_changes_provided_fields(self, schedule):
        schedule.update(enabled=True, top_n=10)
        row = schedule.get()
    ...
    def test_update_ignores_unknown_fields(self, schedule):
        # Caller can pass anything (Pydantic should have stripped); just
        # confirm repo doesn't blow up.
        schedule.update(top_n=7, not_a_real_field="ignored")
        assert schedule.get().top_n == 7

    def test_update_skips_None_values(self, schedule):
        # Patch semantics: None means "leave alone".
        schedule.update(enabled=True)
        schedule.update(enabled=None, top_n=20)
        row = schedule.get()
