...
#          A SINGLE homogeneous LINE-ITEMS source: gross_profit (or revenue -
#          cost_of_revenue) over total_assets. No metrics gross_margin mix (different
#          scaling) — a live smoke confirmed line items populate, metrics did not.
# ===========================================================================
...
def test_gross_prof_homogeneous_no_metrics_fallback():
    # A line item with gross_profit but NO total_assets is not computable; even with a
    # gross_margin on the metrics, gross_prof is STILL None — proving the factor uses
    # ONLY the homogeneous line-items gp/assets source, never the margin-scaled metric.
...
def _smoke_universe() -> dict[str, SimpleNamespace]:
    ...
    * a fixed EPS over a close that compounds faster for high-``i`` makes
    ...
    for i in range(n):
        ...
        items = [
            _smoke_li(_FY, earnings_per_share=4.0, book_value_per_share=20.0, gross_profit=200.0 + i, total_assets=1000.0 + 10.0 * i),
            _smoke_li(_FY_PRIOR, total_assets=900.0 + 10.0 * i),
        ]
        # gross_prof = gross_profit / total_assets off the homogeneous line items; the
        # slight per-name variation keeps it non-degenerate (non-None) across the cross-section.
